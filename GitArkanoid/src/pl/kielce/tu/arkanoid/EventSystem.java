...
public class EventSystem {
	...
	public static void handleMenuEvent(KeyEvent event) {
		if(event.getCode() == KeyCode.DOWN) {
			if(Values.currentMenuOption < 2) Values.currentMenuOption++;
		}
			...
			if(Values.currentMenuOption == 2) Values.gameState = GameState.QUIT; 
		}
	}
	...
	public static void moveBall() {
		...
		if(Values.destroyedBlocks == Values.difficultyLevel.blockCols * Values.difficultyLevel.blockRows)
		{
		...
		for(int i = 0; i < blocks.length; i++) {
			for(int j = 0; j < blocks[i].length; j++) {
				...
				if (
					...
					// ZMIANA WYNIKU W ZALEZNOSCI OD POZIOMU TRUDNOSCI
					if(Values.difficultyLevel.equals(DifficultyLevel.EASY))
						Values.score += 5;
					if(Values.difficultyLevel.equals(DifficultyLevel.MEDIUM))
						Values.score += 10;
					if(Values.difficultyLevel.equals(DifficultyLevel.HARD))
						Values.score += 15;
					if(Values.difficultyLevel.equals(DifficultyLevel.IMPOSSIBLE))
						Values.score += 20;
					
					Values.destroyedBlocks++;
	...
	public static void showScore()
	{
		...
		EventHandler<ActionEvent> event = new EventHandler<ActionEvent>() 
		{
			...
			{
				// set alert type
				e.setAlertType(AlertType.CONFIRMATION);
